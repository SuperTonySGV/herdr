...
- `last_active` — time since the agent last changed state, in the compact form `now`, `27m`, `17h`, `3d`. A working agent always reads `now`. Placed last in a row it is pinned to the sidebar's right edge; anywhere else it flows inline. Rows drop it before truncating the rest of the row, and an agent that has not changed state yet shows nothing.
